...
<form novalidate method="GET" action="{{ route($indexRoute) }}" class="flex flex-wrap items-center gap-3"
    id="qa-board-filter-form">
    ...
    <div role="tablist" aria-label="解決状態フィルタ" class="inline-flex bg-ink-50 rounded-md p-[3px] gap-[2px]">
        @foreach ($segments as $value => $label)
            <button type="submit" name="status" value="{{ $value }}" role="tab"
    ...
    <div class="flex flex-wrap items-center gap-2">
        <a href="{{ route($indexRoute, array_filter(['status' => $currentStatus, 'keyword' => $keyword])) }}"
        ...
        @foreach ($certifications as $cert)
            ...
            <a href="{{ route($indexRoute, array_filter(['certification_id' => $active ? null : $cert->id, 'status' => $currentStatus, 'keyword' => $keyword])) }}"
    ...
    <div class="relative min-w-[220px] max-w-[320px] flex-1">
        <x-icon name="magnifying-glass" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-500"
            aria-hidden="true" />
        <input type="search" name="keyword" value="{{ $keyword }}" maxlength="100" placeholder="質問の本文を検索..."
