...
通知・メールはデータベースキューへ登録し、workerがバックグラウンドで送信します。worker停止中はジョブが蓄積され、起動後に処理されます。
...
プロジェクトのディレクトリで、別ターミナルから起動します。

```bash
sail artisan queue:work database --queue=default --sleep=1
```
...
原因を修正した後、一覧に表示されたUUIDを指定して再投入

```bash
sail artisan queue:retry <失敗ジョブのUUID>
```
